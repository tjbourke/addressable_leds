...
void Led::loop()
{
  ...
  } else if (a == "fast white spark") {
    Spark(strip,frame,245); //"Spark" function without hue make a white spark, 246 fade is faster which makes for a sharper dropoff
  } else if (a == "slow white spark") {
    Spark(strip,frame,255);
    delay(2);       //Slow things down a bit more for Slow Spark
  } else if (a == "rainbow spark") {
...
void Led::SetAnimation(uint8_t anim)
{
  ...
  animation = animation % animationMapLength;
  
  Serial.print("Animation Number: ");
  Serial.println(animation);
  
  if (animationMap[animation]) {
    SetAnimationName(animationMap[animation]);
  }       
}

void Led::SetAnimationName(String animName)
{
